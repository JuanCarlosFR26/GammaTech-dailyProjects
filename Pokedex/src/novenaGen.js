...
body.style.backgroundImage = "url('../assets/img/backOctGen.jpg')"
...
const fetchData = async () => {
  const response = await fetch("https://pokeapi.co/api/v2/pokemon/?limit=103&offset=905");
...
const getPokemons = async () => {
  ...
  pokemonsList.forEach((pokemon) => {
    ...
    const everyPokemon = async () => {
      ...
      localStorage.setItem("pokemon9", JSON.stringify(pokeArr));
    };
...
function pintarPoke() {
  const pokeStorage = JSON.parse(localStorage.getItem("pokemon9"));
