...
body.style.backgroundImage = "url('../assets/img/backTerGen.png')"
...
const getPokemons = async () => {
  ...
  pokemonsList.forEach((pokemon) => {
    ...
    const everyPokemon = async () => {
      ...
      localStorage.setItem("pokemon3", JSON.stringify(pokeArr));
    };
...
function pintarPoke() {
  const pokeStorage = JSON.parse(localStorage.getItem("pokemon3"));
  ...
  const types = typePoke.forEach(type => {
        ...
        if(card.classList.contains('ground') && card.classList.contains('psychic')) {
          card.setAttribute('id', 'ground-psychic');
      }
        ...
        if(card.classList.contains('rock') && card.classList.contains('grass')) {
          card.setAttribute('id', 'rock-grass');
      }

        if(card.classList.contains('rock') && card.classList.contains('psychic')) {
          card.setAttribute('id', 'rock-psychic');
      }
        ...
        if(card.classList.contains('water') && card.classList.contains('grass')) {
          card.setAttribute('id', 'water-grass');
        }

        if(card.classList.contains('bug') && card.classList.contains('ground')) {
          card.setAttribute('id', 'bug-ground');
        }

        if(card.classList.contains('bug') && card.classList.contains('phantom')) {
          card.setAttribute('id', 'bug-phantom');
        }

        if(card.classList.contains('fire') && card.classList.contains('ground')) {
          card.setAttribute('id', 'fire-ground');
        }

        if(card.classList.contains('ground') && card.classList.contains('dragon')) {
          card.setAttribute('id', 'ground-dragon');
        }
    })
